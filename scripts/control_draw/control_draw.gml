function control_draw() {
	...
	if (totalblocks > 0) {
		...
		if (tonextbackup <= 0 && playing == 0) {
			save_song(backup_file + condstr(filename != "", filename_name(filename), song_backupid) + "_backup.nbs", true)
	...
	xx += 25
	if (draw_icon(icons.STOP, xx, yy, "Stop song", 0, 0)) {playing = 0 marker_pos = 0 marker_prevpos = 0 timestoloop = real(loopmax) remove_emitters_all_schedule = 1} xx += 25
	if (isplayer && !dropmode) if (draw_icon(icons.STOP, rw / 2 - 12 - 100, rh / 2 + 50, "Stop song", 0, 0)) {playing = 0 marker_pos = 0 marker_prevpos = 0 timestoloop = real(loopmax) remove_emitters_all_schedule = 1}
	forward = 0
	...
	xx += 25
	if (draw_icon(icons.STOP, xx, yy, "停止歌曲", 0, 0)) {playing = 0 marker_pos = 0 marker_prevpos = 0 timestoloop = real(loopmax) remove_emitters_all_schedule = 1} xx += 25
	if (isplayer && !dropmode) if (draw_icon(icons.STOP, rw / 2 - 12 - 100, rh / 2 + 50, "停止歌曲", 0, 0)) {playing = 0 marker_pos = 0 marker_prevpos = 0 timestoloop = real(loopmax) remove_emitters_all_schedule = 1}
	forward = 0
